<?php
class Number_Format_Cost_Calculator_Widget extends \Elementor\Widget_Base{
	...
	public function get_title() {
		return esc_html__( 'Calculator number', 'cost-calculator-for-elementor' );
	}
	/**
	 * Get widget icon.
	 *
	...
	public function get_keywords() {
		return [ 'currency', 'currencies','number','calculator','cost','formula' ];
	}
	/**
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => 'number-change_name',
			]
		);
		$this->add_control(
			'default',
			[
				'label' => esc_html__( 'Default', 'cost-calculator-for-elementor' ),
	...
	protected function render() {
		...
		?>
		<input <?php echo ($atts) ?> value="<?php echo esc_attr($settings["default"])  ?>" class="<?php echo esc_attr($class) ?>" type="text" name="<?php echo esc_attr($name) ?>" id="<?php echo esc_attr($name) ?>">
		<?php
	}
}